...
router.all('/wordgame/api/v2/:userid*', function (req, res, next) {//validate
...
router.post('/wordgame/api/v2/:userid', function (req, res, next) {
    ...
    game.create(userid, colors, font, "", level, level.guesses, "Unfinished", target, timeStamp, view, function (err, newGame) {
        res.json(newGame);
    });
});
...
router.get('/wordgame/api/v2/:userid/:gid', function (req, res, next) {
    ...
    game.findByUserAndGameID(userId, gameId, function (err, gameInDB) {
        if (err) {
            res.send(err);
        } else {
...
router.post('/wordgame/api/v2/:userid/:gid/guesses', function (req, res, next) {
    ...
    let userId = req.params.userid;
    let gameId = req.params.gid;
    game.findByUserAndGameID(userId, gameId, function (err, resultGame) {
        if (err) {
            res.status(403).send(err);
        } else {
...
router.put('/wordgame/api/v2/:userid/defaults', function (req, res, next) {
    ...
    users.update(userid, defaults, function (err, update) {
        if (err) {
